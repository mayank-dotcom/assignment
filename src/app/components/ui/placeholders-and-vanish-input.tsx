...
// Utility to generate a simple unique userId (or use a library like uuid)
...
export default function PlaceholdersAndVanishInput({
  ...
  interface UserState {
    question: string;
    userId: string; // Add userId to the state
  }
  ...
  const run = async () => {
    try {
      ...
      // Send both question and userId to the backend
      setResponse(" ")
      const result = await axios.post("/api/use/", {
      ...
      if (result.data.text !== undefined) {
        ...
        setResponse("Please wait a few seconds then retry. Either there is a network connectivity issue or you might have hit the hugging face api limit. If reaching the limit is the case than please try next day once the API limit resets.");
  ...
  return (
    ...
    >
      ...
      <input
        ...
        value={user.question}
        type="text"
        placeholder="Press tab once than start typing | Hit enter to prompt"
        className={cn(
      ...
      <div className="absolute inset-0 flex items-center rounded-full pointer-events-none" id="place_holder">
...
:
      <div id="answer_cont" style={{position:"relative"}}>
        <div id="answer_sub_answer"> 
